...
// All data goes through here. Falls back to localStorage if no backend.

const API_BASE = window.SX_API_BASE || 'http://localhost:5000/api';
...
const SxAuth = {
  ...
  clearTokens()        { localStorage.removeItem('sx_access_token'); localStorage.removeItem('sx_refresh_token'); localStorage.removeItem('sx_current_user'); },
...
const SxAPI = {
  async signIn(email, password) {
    const data = await sxFetch('/auth/signin', {
      method: 'POST',
  ...
  async signUp(first, last, email, password, role = 'Analyst') {
    const data = await sxFetch('/auth/signup', {
      method: 'POST',
      body: JSON.stringify({ first, last, email, password, role }),
    });
    SxAuth.setTokens(data.access_token, data.refresh_token);
    SxAuth.setUser(data.user);
    return data.user;
  ...
  async signOut() {
    try {
      await sxFetch('/auth/signout', {
        method: 'POST',
        body: JSON.stringify({ refresh_token: SxAuth.getRefresh() }),
      });
    } catch { /* ignore */ }
    SxAuth.clearTokens();
  },
  ...
  async me() {
    const data = await sxFetch('/auth/me');
    SxAuth.setUser(data.user);
    return data.user;
  },
  ...
  async changePassword(cur, nw) {
    return sxFetch('/users/me/password', {
      method: 'PATCH',
  ...
  // ── VM ──
  async vmList(filters = {})  {
    const params = new URLSearchParams(filters).toString();
    return (await sxFetch('/vm' + (params ? '?' + params : ''))).records;
  },
  ...
  // ── ESC ──
  async escList(filters = {}) {
    const params = new URLSearchParams(filters).toString();
    return (await sxFetch('/esc' + (params ? '?' + params : ''))).records;
  },
...
async function sxDetectBackend() {
  ...
  if (!window.SX_BACKEND_MODE) {
    console.warn('[SentinelX] Backend not reachable — running in localStorage mode');
  } else {
    console.log('[SentinelX] Backend connected at', API_BASE);
  }
